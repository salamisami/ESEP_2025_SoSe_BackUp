...
void test_ins(PM::Sender* sender, PM::Receiver* receiver) {
...
int main() {
    ...
    using namespace PM;
    Receiver eventQueue;
    Sender interruptS(eventQueue.getChannelID());

    Receiver actuatorR;
    Sender controller(actuatorR.getChannelID());
    
    Interrupt interrupt(&interruptS);
    Actuator actuator(&actuatorR);

    std::cout << "Festo is gate: " << std::boolalpha << actuator.isGate() << std::endl;
    //actuator.test_outs();

    test_ins(&controller, &eventQueue);
    //wait(10);
