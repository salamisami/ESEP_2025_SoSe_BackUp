...
TEST_F(FBM2TestSetup, FBM2Test)
{
    ...
    double position = 80;
    data->piece_tracker->update_distance_force(Area::START_ADC, position);
    WAIT(110);
    ...
    data->piece_tracker->update_distance_force(Area::ADC_GATE, position);
    ...
    data->piece_tracker->update_distance_force(Area::GATE_END, position);
    remote_control->send_event((int8_t) Topic::INTERRUPT, (int) InterruptEnum::LASER_BACK_BLOCKED);
