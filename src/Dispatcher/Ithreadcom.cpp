...
Ithread_com::Ithread_com(const char* name) {
	gns_name = name;
...
void Ithread_com::send_event(int8_t event_code,int event_value,int priority){
	MsgSendPulse(coid, priority, event_code, event_value);
}

name_attach_t* Ithread_com::get_attach(){
	return attach;
}

int  Ithread_com::get_coid(){
	return coid;
}

const char* Ithread_com::get_name(){
	return gns_name;
}
//-------------------------------Receive Qnet Msgs--------------------------------------------//
int Ithread_com::receive_event(_pulse* event){
...
void Ithread_com::handle_QNX_IO_msg(_pulse* msg,int rcvid){
	switch (msg->code) {
...
void Ithread_com::handle_app_msg(_pulse* msg,int rcvid){
	printf("%s: Unexpected message type 0x%04X\n", gns_name, msg->type);
	MsgError(rcvid,EPERM);
}

void Ithread_com::handle_QNX_pulse(_pulse* msg,int rcvid){
	switch (msg->code) {
		case _PULSE_CODE_DISCONNECT:
			printf("%s _PULSE_CODE_DISCONNECT\n", gns_name);
/* A client disconnected all its connections (called
