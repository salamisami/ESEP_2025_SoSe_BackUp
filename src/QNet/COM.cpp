...
void COM::start()
{
    ...
    udpWatchdogThread = std::thread([this]
                                    { runUdpWatchdog(); });
}
...
void COM::runServer()
{
    ...
    while (running)
    {
        ...
        if (rcvid == 0)
        {
            ...
            {
                if (event.code == _PULSE_CODE_DISCONNECT || event.code == _PULSE_CODE_COIDDEATH)
                {
                    ConnectDetach(event.scoid);
                    std::cout << "Server: Disconnect oder COIDDEATH erkannt und detach ausgeführt." << std::endl;
                }
                continue;
            }
        } else if(rcvid == -1) {
...
void COM::runUdpWatchdog()
{
    ...
    {
        sendto(sockfd, msg, sizeof(msg), 0, (sockaddr *)&peerAddr, sizeof(peerAddr));
        ...
        if (rv > 0 && FD_ISSET(sockfd, &readfds))
        {
            ...
            int n = recvfrom(sockfd, buf, sizeof(buf), 0, (sockaddr *)&fromAddr, &fromLen);
            if (n > 0)
            {
        ...
        auto now = std::chrono::steady_clock::now();
        auto ms_since = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastReceived).count();
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(UDP_WATCHDOG_PERIOD_MS));
    }
...
void COM::notifyUdpRestored()
{
    ...
    std::cout << "[UDP-Watchdog] Verbindung zur Gegenseite wiederhergestellt!" << std::endl;
    _pulse reconnectEvent;
    reconnectEvent.code = static_cast<int8_t>(Topic::COM);
    reconnectEvent.value.sival_int = static_cast<int>(COM_Enum::COM_CONNECTED);
    sendToDispatcher(reconnectEvent, static_cast<int>(EventPriority::FIRST_PRIO));
