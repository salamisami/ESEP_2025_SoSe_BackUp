...
void COM::sendHeartbeat() {
  ...
  const auto elapsed =
      std::chrono::duration_cast<std::chrono::nanoseconds>(now - lastHeartbeat);
...
void COM::runServer() {
  ...
  try {
    ...
    SIGEV_PULSE_INIT(&event, coid, SIGEV_PULSE_PRIO_INHERIT, (int)Topic::COM,
                     (int)COM_Enum::TIMEOUT_COM);
    ...
    while (running) {
      ...
      if (rcvid == 0) { // Pulse received
        if (pulse.value.sival_int == (int)COM_Enum::TIMEOUT_COM) {
        ...
        if (disconnected) {
          ...
          rampEvent.value.sival_int =
              rampfull ? static_cast<int>(COM_Enum::RAMP_FULL)
        ...
        if ((_PULSE_CODE_MINAVAIL <= pulse.code) &&
            (pulse.code <= _PULSE_CODE_MAXAVAIL)) {
          updateHeartbeat();
