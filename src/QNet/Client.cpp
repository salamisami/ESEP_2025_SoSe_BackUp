...
int Client::sender() { // Sender side of the code
	int receiver_coid;
...
void Client::handle_QNX_pulse(header_t hdr, int rcvid){
	switch (hdr.code) {
...
void Client::handle_appl_pulse(header_t hdr, int rcvid){
	switch (hdr.code) {
	case MY_PULSE_CODE:
...
void Client::handle_pulse(header_t hdr, int rcvid){
...
void Client::handle_ONX_IO_msg(header_t hdr, int rcvid){
...
void Client::handle_app_msg(header_t hdr, int rcvid){
	printf("Receiver: Unexpected message type 0x%04X\n", hdr.type);
	MsgError(rcvid,EPERM);
}

int Client::receiver() { // Receiver side of the code
...
int Client::qnet_init(int argc,const char argv) {
	if (argc < 2) {
		printf("Usage %s -s | -r \n", argv);
		return EXIT_FAILURE;
	} else if (argv == 's') {
