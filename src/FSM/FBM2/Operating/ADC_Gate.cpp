...
State* ADC_Gate::laser_sorting_gate_blocked() {
	if 	((data->piece_tracker->get_distance().second >= WAY_TO_AREA && data->piece_tracker->get_distance().first == Area::ADC_GATE) ||
		(data->piece_tracker->get_distance().second <= OVER_AREA && data->piece_tracker->get_distance().first == Area::GATE)) {
...
State* ADC_Gate::timer(TIMER_ID id) {
	if(id == TIMER_ID::ADC_GATE) {
		if (data->piece_tracker->get_distance().second >= OVER_AREA && data->piece_tracker->get_distance().first == Area::GATE) {
