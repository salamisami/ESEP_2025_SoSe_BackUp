...
template <typename T> class Context {
  static_assert(std::is_base_of<State, T>::value,
                "Template parameter must inherit from State");
...
template <typename T> Context<T>::~Context() { delete state; }

//===================================================== private functions
//=====================================================

template <typename T> State *Context<T>::handleInternal(int event_value) {
...
template <typename T> State *Context<T>::handleADC(int event_value) {
...
template <typename T> State *Context<T>::handleCOM(int event_value) {
  State *newState = nullptr;
...
template <typename T> State *Context<T>::handleInterrupt(int event_value) {
  State *newState = nullptr;
...
template <typename T> State *Context<T>::handlePiece(int event_value) {
  State *newState = nullptr;
...
template <typename T> State *Context<T>::handleRemote(int event_value) {
  State *newState = nullptr;
...
template <typename T> State *Context<T>::handleError(int event_value) {
  State *newState = nullptr;
...
//===================================================== public functions
//=====================================================
template <typename T> std::string Context<T>::show_state() {
  return state->get_current_state();
}

template <typename T> void Context<T>::handleEvent(_pulse event) {
  State *newState = nullptr;
  Topic event_code = (Topic)event.code;
  int event_value = event.value.sival_int;
  data->event_payload = event_value;
  data->motorpieceRequest = (int8_t)event_code;
  switch (event_code) {
